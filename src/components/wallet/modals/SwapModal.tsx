      ...
      <div className="p-6 flex-1 min-h-0 overflow-y-auto">
          ...
          <div>
            ...
            <div className="mb-2">
              <div className="flex justify-between items-center mb-2">
                <span className="text-xs font-medium text-[#ffe2cc]">From</span>
                {fromAsset && (
                  <span className="text-xs text-[#ffe2cc] truncate ml-2">
                    Balance: <span className="text-white">{formatAssetAmount(fromAsset)}</span>
                  </span>
                )}
              </div>
                ...
                <div className="flex items-center gap-2">
                  <div className="relative shrink-0">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2 py-2 bg-white/6 border border-white/10 rounded-xl hover:bg-white/6 transition-colors whitespace-nowrap"
                    >
                      {fromAsset ? (
                        <>
                          {fromAsset.iconUrl && <img src={fromAsset.iconUrl} className="w-5 h-5 rounded-full shrink-0" alt="" />}
                          <span className="text-white font-medium text-sm">{fromAsset.symbol}</span>
                        </>
                      ) : (
                    ...
                    {showFromDropdown && (
                      <div className="absolute top-full left-0 mt-2 w-48 bg-white/6 border border-white/10 rounded-xl shadow-xl z-50 max-h-48 overflow-y-auto">
                        {assets.map((asset) => (
                          <button
                            key={asset.coinId}
                            onClick={() => { setFromAsset(asset); setShowFromDropdown(false); setFromAmount(''); }}
                            className="w-full flex items-center gap-3 p-3 hover:bg-white/6 transition-colors text-left"
                          >
                            {asset.iconUrl && <img src={asset.iconUrl} className="w-6 h-6 rounded-full" alt="" />}
                            <div className="flex-1 min-w-0">
                              <div className="text-white font-medium text-sm truncate">{asset.symbol}</div>
                              <div className="text-xs text-[#ffe2cc] truncate">{formatAssetAmount(asset)}</div>
                            </div>
                          </button>
                        ))}
                      </div>
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    disabled={!fromAsset}
                    className="flex-1 bg-transparent text-right text-xl font-mono text-white outline-none disabled:opacity-50 min-w-0"
                  />
                </div>
                {fromAsset && fromAmount && (
                  <div className="mt-2 text-right text-xs text-[#ffe2cc]">
            ...
            <div className="flex justify-center items-center mt-4 mb-1 relative z-10">
              <button onClick={handleFlipAssets} className="p-2 bg-white/4 border-2 border-white/10 rounded-full hover:bg-white/6 transition-colors">
                <ArrowDownUp className="w-5 h-5 text-[#ffe2cc]" />
              </button>
            </div>
            ...
            <div className="mb-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-xs font-medium text-[#ffe2cc]">To</span>
              </div>
              <div className="bg-white/4 rounded-2xl p-3">
                <div className="flex items-center gap-2">
                  <div className="relative shrink-0">
                    ...
                    >
                      {toAsset ? (
                        <>
                          ...
                          <span className="text-white font-medium text-sm">{toAsset.symbol}</span>
                        </>
                      ) : (
                        <span className="text-neutral-500 text-sm">Select</span>
                      )}
                    ...
                    {showToDropdown && (
                      <div className="absolute bottom-full left-0 mb-2 w-48 bg-white/6 border border-white/10 rounded-xl shadow-xl z-50 max-h-48 overflow-y-auto">
                  ...
                  <div className="flex-1 text-right text-xl font-mono text-white break-all min-w-0">
            ...
            {exchangeInfo && fromAsset && toAsset && (
              <div className="mb-4 p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-blue-400">Exchange Rate</span>
                </div>
        ...
        {step === 'processing' && (
          <div className="py-10 text-center">
            ...
            <h3 className="text-white font-medium text-lg">Processing Swap...</h3>
        ...
        {step === 'success' && fromAsset && toAsset && exchangeInfo && (
          <div className="py-10 text-center">
            ...
            <h3 className="text-white font-bold text-2xl mb-2">Swap Complete!</h3>
            <p className="text-[#ffe2cc] mb-1">
            ...
            <button
              onClick={handleClose}
              className="mt-8 px-8 py-2 bg-white/6 rounded-lg hover:bg-white/8 text-white transition-colors"
            >
              Close
            </button>
          </div>
