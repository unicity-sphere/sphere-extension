    ...
    <WalletScreen isOpen={isOpen} onClose={handleClose}>
      ...
      <div className="p-4 flex-1 flex flex-col justify-center overflow-y-auto">
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full bg-white/4 border border-white/10 rounded-xl py-2.5 pr-4 text-white focus:border-brand-orange outline-none text-sm ${recipientMode === 'nametag' ? 'pl-7' : 'pl-3 font-mono'}`}
              ...
              <button
                ...
                className="text-xs text-neutral-400 hover:text-brand-orange mt-1.5 transition-colors"
              >
            ...
            {availableCoins.map((coin) => (
              ...
              >
                {coin.iconUrl ? (
                  ...
                  <div className="w-8 h-8 rounded-full bg-white/8 flex items-center justify-center text-xs font-bold text-neutral-500">
        ...
        {step === 'amount' && selectedCoin && (
          <div>
            <div className="mb-4">
              <label className="text-sm text-[#ffe2cc] block mb-2">Amount ({selectedCoin.symbol})</label>
              <input
                autoFocus
                type="text"
                inputMode="decimal"
                ...
                className="w-full bg-white/4 border border-white/10 rounded-xl py-3 px-4 text-white text-2xl font-mono outline-none focus:border-brand-orange"
                placeholder="0.00"
              />
            </div>
            <div className="mb-4">
              <label className="text-sm text-[#ffe2cc] block mb-2">Message (optional)</label>
              <input
                type="text"
                value={messageInput}
                onChange={(e) => setMessageInput(e.target.value)}
                className="w-full bg-white/4 border border-white/10 rounded-xl py-2.5 px-4 text-white outline-none focus:border-brand-orange text-sm"
        ...
        {step === 'confirm' && selectedCoin && (
          <div>
            <div className="bg-white/4 rounded-xl p-4 mb-4 border border-white/10 text-center">
              <div className="text-xs text-neutral-500 mb-1">You are requesting</div>
              <div className="text-2xl font-bold text-white mb-3">
                {amountInput} <span className="text-brand-orange">{selectedCoin.symbol}</span>
              </div>
              <div className="text-xs text-neutral-500 mb-1">from</div>
              <div className="flex items-center justify-center gap-2 text-sm bg-white/6 p-2 rounded-lg mx-auto max-w-max">
            ...
            <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl flex items-start gap-2">
              ...
              <div>
                <div className="text-blue-400 text-xs font-medium">Payment Request</div>
        ...
        {step === 'processing' && (
          <div className="py-10 text-center">
            ...
            <p className="text-white font-medium">Sending Payment Request...</p>
        ...
        {step === 'success' && selectedCoin && (
          <div className="py-8 text-center">
            ...
            <p className="text-[#ffe2cc] text-sm">
              Payment request for <b>{amountInput} {selectedCoin.symbol}</b> sent to{' '}
              <b>{recipientMode === 'direct' ? recipient : `@${recipient}`}</b>
            </p>
            <button
              onClick={handleSuccessClose}
              className="mt-6 px-6 py-2 bg-white/6 rounded-lg hover:bg-white/8 text-white transition-colors text-sm"
            >
              Close
            </button>
          </div>
