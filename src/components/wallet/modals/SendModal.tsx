    ...
    <WalletScreen isOpen={isOpen} onClose={handleClose}>
      ...
      <div className="px-6 py-3 flex-1 flex flex-col justify-center overflow-y-auto">
        ...
        {step === 'recipient' && (
          <div>
            <div className="mb-6">
              <label className="text-sm text-[#ffe2cc] block mb-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full bg-white/4 border border-white/10 rounded-xl py-3 pr-4 text-white focus:border-brand-orange outline-none ${recipientMode === 'nametag' ? 'pl-8' : 'pl-4 font-mono text-sm'}`}
                  placeholder={recipientMode === 'nametag' ? 'Unicity ID' : 'DIRECT://...'}
                />
              </div>
              {recipientError && <p className="text-red-400 text-sm mt-2">{recipientError}</p>}
              <button
                onClick={() => { setRecipientMode(recipientMode === 'nametag' ? 'direct' : 'nametag'); setRecipient(''); setRecipientError(null); }}
                className="text-[11px] text-neutral-500 hover:text-brand-orange mt-2 transition-colors"
              >
          ...
          <div className="space-y-2">
            {assets.map((asset) => (
              <button
                ...
                className="w-full p-3 flex items-center gap-3 bg-white/4 hover:bg-white/6 border border-white/5 rounded-xl transition-colors text-left"
              >
                <img src={asset.iconUrl || ''} className="w-8 h-8 rounded-full" alt="" />
                <div className="flex-1">
                  <div className="text-white font-medium">{asset.symbol}</div>
                  <div className="text-xs text-[#ffe2cc]">{formatAmount(asset.totalAmount, asset.decimals)} available</div>
                </div>
          ...
          <div>
            <div className="mb-6">
              <div className="flex justify-between text-sm mb-2">
                <span className="text-[#ffe2cc]">Amount</span>
                <span className="text-[#ffe2cc]">
                  Available: <span className="text-white">{formatAmount(selectedAsset.totalAmount, selectedAsset.decimals)}</span>
                </span>
              </div>
              <div className="relative">
                <input
                  ...
                  className={`w-full bg-white/4 border rounded-xl py-3 px-4 text-white text-2xl font-mono outline-none ${insufficientBalance ? 'border-red-500 focus:border-red-500' : 'border-white/10 focus:border-brand-orange'}`}
                  placeholder="0.00"
                />
                <button
                  onClick={() => setAmountInput(formatAmount(selectedAsset.totalAmount, selectedAsset.decimals))}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-xs bg-white/6 text-brand-orange px-2 py-1 rounded hover:bg-white/8"
                >
                  MAX
                </button>
              </div>
              {insufficientBalance && <p className="text-red-400 text-sm mt-2">Insufficient balance</p>}
              {recipientError && <p className="text-red-400 text-sm mt-2">{recipientError}</p>}
            </div>
            <div className="mb-6">
              <label className="text-sm text-[#ffe2cc] block mb-2">Memo (optional)</label>
              <input
                type="text"
                value={memoInput}
                onChange={(e) => setMemoInput(e.target.value)}
                className="w-full bg-white/4 border border-white/10 rounded-xl py-3 px-4 text-white outline-none focus:border-brand-orange text-sm"
        ...
        {step === 'confirm' && selectedAsset && (
          <div>
            ...
            {/* Summary Card */}
            <div className="bg-white/4 rounded-2xl p-5 mb-6 border border-white/10 text-center">
              <div className="text-sm text-[#ffe2cc] mb-1">You are sending</div>
              <div className="text-3xl font-bold text-white">
                {amountInput} <span className="text-brand-orange">{selectedAsset.symbol}</span>
              </div>
              ...
              <div className="flex flex-col items-center gap-1.5">
                <div className="flex items-center gap-1.5">
                  <div className="flex items-center gap-2 text-sm bg-white/6 p-2 rounded-lg">
                    {recipientMode === 'direct' ? (
                      <Hash className="w-4 h-4 text-[#ffe2cc]" />
                    ) : (
                      <User className="w-4 h-4 text-[#ffe2cc]" />
                    )}
                    <span className={`text-neutral-300 ${recipientMode === 'direct' ? 'font-mono text-xs break-all' : ''}`}>
                ...
                {recipientMode === 'nametag' && resolvedAddress && (
                  <div className="flex items-center gap-1">
                    <span
                      className="text-[10px] font-mono text-neutral-500/50 truncate max-w-52"
                    ...
                    <button
                      ...
                      className="p-0.5 hover:bg-white/8 rounded transition-colors"
                      title="Copy address"
                    >
                      {copiedKey === 'address'
              ...
              {memoInput && (
                <div className="text-sm text-[#ffe2cc] mt-3 italic">
            ...
            <div className="mb-6 space-y-2">
              <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-start gap-3">
                ...
                <div>
                  <div className="text-emerald-400 text-sm font-medium">Smart Transfer</div>
                  <div className="text-xs text-[#ffe2cc] mt-1">
            ...
            {recipientError && <p className="text-red-400 text-sm mb-4 text-center">{recipientError}</p>}
        ...
        {step === 'success' && (
          <div className="py-10 text-center">
            ...
            <p className="text-[#ffe2cc]">
              Successfully sent <b>{amountInput} {selectedAsset?.symbol}</b> to <b>{recipientMode === 'direct' ? recipient : `@${recipient}`}</b>
            </p>
            <button onClick={handleSuccessClose} className="mt-8 px-8 py-2 bg-white/6 rounded-lg hover:bg-white/8 text-white transition-colors">
              Close
            </button>
          </div>
        )}
