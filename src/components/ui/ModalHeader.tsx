...
const iconVariantClasses: Record<IconVariant, { badge: string; icon: string }> = {
  gradient: {
    badge: 'bg-linear-to-br from-brand-orange to-brand-orange-dark shadow-lg shadow-brand-orange/30',
    icon: 'text-white',
  },
  neutral: {
    badge: 'bg-white/6',
...
export function ModalHeader({
  ...
  return (
    <div className="relative z-10 px-6 py-3 border-b border-white/10 flex justify-between items-center shrink-0">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {subtitle && (
            <div className="text-xs text-[#ffe2cc]">{subtitle}</div>
          )}
        </div>
      </div>
      ...
      <button
        ...
        className={`w-9 h-9 flex items-center justify-center rounded-xl transition-colors ${
          closeDisabled
            ? 'bg-white/4 text-neutral-600 cursor-not-allowed'
            : 'hover:bg-white/8 text-neutral-400 hover:text-white'
        }`}
      >
        <X className="w-4 h-4" />
      </button>
